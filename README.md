...
- 使用するブランチ: feature/ainsert-structured-input ← これを使っている
...
- 使用するブランチ: feature/ainsert-structured-input
  - refactor-insertion-pipeline をベースに「doing.md」の9/21 のやつを実装したもの。
    ↑
...
- ソースコードの反映をする(基本はこれ): `sudo scripts/start.sh cleanup`
  ...
  - ソースコードの変更だけならビルドはすぐに終わるようになっている
...
# マルチモーダルは想定しておらず、扱えるのはテキストのみ

コードを確認した結果、MiniRAGはテキスト情報のみを処理するように設計されていることを確認しました。

その理由は以下の通りです：

テキストベースの入力：コアのデータ取り込み機能（ainsert）は、テキストの文字列を受け入れるように設計されており、画像ファイルやデータは受け付けません。
テキスト専用の処理：全体の処理パイプラインはテキスト専用に設計されています。例えば、tiktokenライブラリを使用してテキストを小さなチャンクに分割し、「エンティティ抽出」というテキスト分析技術を実施しています。
画像ライブラリなし：画像処理に関連するコードやライブラリ（PillowやOpenCVなど）は見つかりませんでした。このシステムは画像コンテンツを理解したり分析したりする機能を備えていません。
要するに、MiniRAGは古典的なテキストベースのRAGアプリケーションであり、画像ベースの検索をサポートしていません。
